...
# v1 is faster on GPU
def get_graph_feature_v1(x: Tensor, k: int, idx: Tensor) -> Tensor:
...
# v2 is faster on CPU
def get_graph_feature_v2(x: Tensor, k: int, idx: Tensor) -> Tensor:
    batch_size, num_dims, num_points = x.size()
...
class EdgeConvBlock(nn.Module):
    ...
    def __init__(self, k: int, in_feat: int, out_feats: int, batch_norm: bool=True, activation: bool=True, cpu_mode: bool=False):
...
class ParticleNet(nn.Module):
    ...
    def forward(self, points: Tensor, features: Tensor, mask: Optional[Tensor] = None) -> Tensor:
#         print('points:\n', points)
#         print('features:\n', features)
        if mask is None:
...
class ParticleNetTagger(nn.Module):
    ...
    def reshape_ragged_features(self, features: Tensor, batch_input_count: Tensor, feature_dims: int, padding_value: int, padding_length: int) -> Tensor:
        split_list_feature_tensors = [features]
        if batch_input_count.size(dim = -1) > 1:
            split_list_feature_tensors = torch.tensor_split(features, batch_input_count[:-1])
    ...
    def forward(self, pf_points: Tensor, pf_features: Tensor, pf_mask: Tensor, sv_points: Tensor, sv_features: Tensor, sv_mask: Tensor, batch_count_pf_points: Tensor, batch_count_pf_features: Tensor, batch_count_pf_mask: Tensor, batch_count_sv_points: Tensor, batch_count_sv_features: Tensor, batch_count_sv_mask: Tensor) -> Tensor:
        # if self.for_inference:
        pf_points = self.reshape_ragged_features(pf_points, batch_count_pf_points, self.pf_points_dims, 0, self.input_length.get('pf_points', 100))
        pf_features = self.reshape_ragged_features(pf_features, batch_count_pf_features, self.pf_features_dims, 0, self.input_length.get('pf_features', 100))
        pf_mask = self.reshape_ragged_features(pf_mask, batch_count_pf_mask, 1, 0, self.input_length.get('pf_mask', 100))
        sv_points = self.reshape_ragged_features(sv_points, batch_count_sv_points, self.sv_points_dims, 0, self.input_length.get('sv_points', 10))
        sv_features = self.reshape_ragged_features(sv_features, batch_count_sv_features, self.sv_features_dims, 0, self.input_length.get('sv_features', 10))
        sv_mask = self.reshape_ragged_features(sv_mask, batch_count_sv_mask, 1, 0, self.input_length.get('sv_mask', 10))
