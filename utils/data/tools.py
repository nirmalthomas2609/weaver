...
def _batch_knn(supports, queries, k, maxlen_s, maxlen_q=None, n_jobs=1):
    ...
    for i, (s, q) in enumerate(zip(supports, queries)):
        batch_knn_idx[i, :len(q[:maxlen_q]), :] = _knn(s[:maxlen_s], q[:maxlen_q], k, n_jobs=n_jobs).reshape((-1, k))  # (len(q), k)
...
def _eval_expr(expr, table):
    ...
    tmp.update({'math': math, 'np': np, 'awkward': awkward,
                '_concat': _concat, '_stack': _stack, '_pad': _pad, '_clip': _clip,
                '_batch_knn': _batch_knn, '_batch_permute_indices': _batch_permute_indices, '_batch_argsort': _batch_argsort, '_batch_gather': _batch_gather})
